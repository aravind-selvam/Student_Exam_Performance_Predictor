...
from student.entity.config_entity import *
from student.entity.raw_data_validation import IngestedDataValidation
from student.entity.artifact_entity import DataValidationArtifact, DataIngestionArtifact
    ...
    def is_Validation_successfull(self):
        try:
            ...
            if self.isFolderPathAvailable() == True:
                ...
                logging.info(f"Train_set status'' {train_filename}''''''{is_train_filename_validated}''''''{is_train_column_numbers_validated}''''{is_train_column_name_same}''''{is_train_missing_values_whole_column}")
                logging.info(f"Test_set status'' {test_filename}''''''{is_test_filename_validated}''''''{is_test_column_numbers_validated}''''{is_test_column_name_same}''''{is_test_missing_values_whole_column}")
                ...
                if is_test_filename_validated & is_test_column_numbers_validated & is_test_column_name_same & is_test_missing_values_whole_column:
                    ...
                    raise ValueError("Check your Testing data! Validation failed")
